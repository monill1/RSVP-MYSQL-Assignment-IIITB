...
-- Query to count the number of null in each column
...
-- country, worlwide_gross_income, production_company and languages columns have NULL values
...
-- USing genre table to find movies which belong to one genre
...
-- Action genre has the highest duration of 112.88 min followed by romance and crime genres
...
-- use min and max function for the query

SELECT min(avg_rating) as min_avg_rating,
       max(avg_rating) as max_avg_rating,
       min(total_votes) as mix_total_votes,
...
+------------------+-------------------+---------------------+*/
-- Type your code below:
-- Finding the ramk of production company based on movie count with average rating > 8 using rank function
